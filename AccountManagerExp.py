...
API_ID = 'APPID'
API_HASH = 'APIHASH'
DEFAULT_CHANNEL_ID = channelid # number without ""
CHANNEL_ID_FILE = "channel_id" # just a file name
PHONE_NUMBER = phonenumber # number without ""
...
async def get_attachments(update: Update, context: CallbackContext) -> None:
    try:
        ...
        messages = await client.get_messages(entity, limit=limit, filter=InputMessagesFilterDocument)
        messages = list(messages)
        file_count = sum(1 for message in messages if message.file)

        initial_message = await update.message.reply_text("File download in progress...")

        for i, message in enumerate(messages, start=1):
            if message.file:
                file_path = os.path.join(".", message.file.name)
                progress_bar = 0
                ...
                async def progress_callback(current, total):
                    ...
                    if progress_bar != percentage:
                        progress_bar = percentage

                        await context.bot.edit_message_text(
                            chat_id=update.effective_chat.id,
                            message_id=initial_message.message_id,
                            text=f"Downloading file {i} of {file_count}: {percentage}%"
                        )

                await client.download_media(message, file=file_path, progress_callback=progress_callback)

        await update.message.reply_text("All files have been downloaded.", reply_to_message_id=update.message.message_id)
    except Exception as e:
        await update.message.reply_text(f"Error: {e}", reply_to_message_id=update.message.message_id)
